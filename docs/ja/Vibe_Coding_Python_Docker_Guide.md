...
今まですべての行を自分で入力してPythonコードを書いてきました。でも、欲しいものを平易な日本語で説明するだけでコードが現れたらどうでしょう？Vibe Codingはコンピュータとの会話のようなものです—結果を説明し、Claude Codeがそれを構築し、テストして改良します。魔法ではありません。あなたがビジョンを導き、AIが実装を処理する新しい作業方法です。このチュートリアルでは、自然言語リクエストだけを使って古典的なIrisフラワーデータセットを分析する方法を紹介します。
...
- **[Irisデータセット](https://scikit-learn.org/stable/datasets/toy_dataset.html#iris-dataset)** - 3種の150のアイリスの花の測定値を含む古典的なデータセット
...
Claudeはプロジェクトが何についてかを知っているとより良く機能します。シンプルなコンテキストファイルを作成しましょう。

- Claude Codeターミナルで入力：

```
CLAUDE.mdというファイルを作成して、以下の内容で：このプロジェクトはPythonを使ってIrisフラワーデータセットを分析します。データ操作にpandas、可視化にmatplotlibを使用します。vibe codingを練習しています - 欲しいものを平易な日本語で説明します。
```

- Enterを押す
- Claudeがファイルを作成します
- このコンテキストによりClaudeがプロジェクトの目標を理解するのに役立ちます

## ステップ6：最初のVibe - データを読み込む
...
- Setosaはバージニカよりはるかに小さい花びらを持っている

この探索によりデータのパターンを理解できます。

## ステップ8：3番目のVibe - ヒストグラムを作成
...
- GitHub Desktopを開く
- すべての新しいファイルがリストされている（Pythonスクリプトと PNG 画像）
