...
今まですべての行を自分で入力してRコードを書いてきました。でも、欲しいものを平易な日本語で説明するだけでコードが現れたらどうでしょう？Vibe Codingはコンピュータとの会話のようなものです—結果を説明し、Claude Codeがそれを構築し、テストして改良します。魔法ではありません。あなたがビジョンを導き、AIが実装を処理する新しい作業方法です。このチュートリアルでは、自然言語リクエストだけを使って実際のNBA統計ダッシュボードを構築する方法を紹介します。
...
```
Claude.meというファイルを作成して、以下の内容で：このプロジェクトはRとShinyを使ってNBA統計ダッシュボードを構築します。データにhoopRパッケージを使用し、vibe codingを練習しています。
```
...
## ステップ8：3番目のVibe - 基本的なShinyアプリを作成
...
- Shinyアプリが自動的にリロード（または**Run Shiny App**を再度クリック）
- ブラウザを更新
...
各成功した機能の後：
1. ブラウザでテスト
2. 動作したら、GitHub Desktopでコミット
3. 壊れたら、Claudeにエラーを伝えて修正を依頼
4. 修正されたら、動作バージョンをコミット

## ステップ14：Vibe Codingマインドセット

新しいプログラミング方法を体験しました。何が起こったか：

**従来のコーディング：**
1. hoopRパッケージのドキュメントを調査
2. データ構造を理解
3. Shiny構文を学ぶ
4. UIコードを行ごとに書く
5. エラーをデバッグ
...
動作するダッシュボードができたので、vibe codingを練習しましょう：

**初心者チャレンジ：**
- 「お気に入りのチームの色に合わせて配色を変更して」
- 「テーブルにプレイヤーのポジションを追加して」
- 「リバウンドだけではなく総リバウンドを表示して」

**中級チャレンジ：**
- 「チームレベルの統計を表示する2番目のタブを作成して」
- 「どのポジションが最も得点するかを示すヒートマップを追加して」
- 「散布図のポイントサイズとしてフィールドゴール率を含めて」

**上級チャレンジ：**
- 「試合ごとのデータを追加してシーズン中のトレンドを表示して」
- 「2人のプレイヤーを並べて比較する比較ビューを作成して」
- 「フィルタリングされたデータをCSVとしてエクスポートするダウンロードボタンを追加して」
